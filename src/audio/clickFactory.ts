...
export function clickSpecFor(level: ClickLevel, sound: SoundType): ClickSpec | null {
...
export interface ScheduledClick {
  time: number;
  cancel: (now: number, fadeSeconds?: number) => void;
  disconnect: () => void;
}

export function scheduleClick(ctx: AudioContext, masterGain: GainNode, time: number, spec: ClickSpec, sound: SoundType, onEnded?: (click: ScheduledClick) => void): ScheduledClick {
  ...
  const handle: ScheduledClick = {
    time,
    cancel(now, fadeSeconds = 0.008) {
      if (disconnected) return;
      // A source not yet started must never leak through the fading output.
      try { osc.stop(time > now ? now : now + fadeSeconds); } catch { /* Already ended. */ }
    },
    disconnect() {
      if (disconnected) return;
      disconnected = true;
