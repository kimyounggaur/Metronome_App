...
export const SettingsSheet = memo(function SettingsSheet({ settings, setSettings }: SettingsSheetProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-2">
        ...
        <Toggle label="햅틱" checked={settings.haptics} disabled={!("vibrate" in navigator)} onChange={(checked) => setSettings((current) => ({ ...current, haptics: checked }))} icon={<Bell size={17} aria-hidden="true" />} />
        {!("vibrate" in navigator) ? <p className="text-sm text-[color:var(--muted)]">이 브라우저는 햅틱 진동을 지원하지 않습니다.</p> : null}
        <Toggle label="재생 중 화면 항상 켜기" checked={settings.wakeLock} disabled={!("wakeLock" in navigator)} onChange={(checked) => setSettings((current) => ({ ...current, wakeLock: checked }))} />
        {!("wakeLock" in navigator) ? <p className="text-sm text-[color:var(--muted)]">이 브라우저는 화면 켜짐 유지를 지원하지 않습니다.</p> : null}
        <Toggle label="모바일 가로 전환 시 공연 모드" checked={settings.autoLandscape} onChange={(checked) => setSettings((current) => ({ ...current, autoLandscape: checked }))} />
      </section>
...
function Toggle({ label, checked, onChange, icon, disabled=false }: { label: string; checked: boolean; onChange: (checked: boolean) => void; icon?: ReactNode; disabled?:boolean }) {
  return (
    <label className="flex min-h-12 items-center justify-between gap-3 rounded-2xl px-3 surface-2">
      ...
      <input className="h-5 w-5 shrink-0" type="checkbox" checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} />
    </label>
  );
}
